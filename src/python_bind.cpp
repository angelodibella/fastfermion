...
struct pybind11::detail::type_caster<ankerl::unordered_dense::map<Key, Value, Hash, Equal, Alloc>>
    : pybind11::detail::map_caster<ankerl::unordered_dense::map<Key, Value, Hash, Equal, Alloc>,
                                   Key, Value> {};
...
auto CSCMatrix_to_scipy(const CSCMatrix<ff_complex>& sm) {
    ...
    return csc_matrix_type(
               std::make_tuple(std::move(sm.data), std::move(sm.indices), std::move(sm.indptr)),
               std::make_tuple(sm.shape[0], sm.shape[1]))
        .release();
};
...
void add_poly_basic_ops(py::class_<PolyT>& pyPolyClass) {
    ...
    pyPolyClass.def("commutator",
                    py::overload_cast<const ff_complex&>(&PolyT::commutator, py::const_));
    pyPolyClass.def("commutator",
                    py::overload_cast<const StringT&>(&PolyT::commutator, py::const_));
    ...
    pyPolyClass.def(
        "norm",
        [](const PolyT& p, const std::variant<int, std::string>& v) {
...
void add_pauli_sparse(py::class_<PauliOpT>& pyPauliOpClass) {
    pyPauliOpClass.def(
        "sparse",
        [](const PauliOpT& a, const std::optional<int>& n, const std::optional<int>& nup) {
...
void add_fermi_sparse(py::class_<FermiOpT>& pyFermiOpClass) {
    pyFermiOpClass.def(
        "sparse",
        [](const FermiOpT& a, const std::optional<int>& n, const std::optional<int>& nocc) {
            int _n = n.has_value() ? n.value() : a.extent();
            if (!nocc.has_value()) {
                return CSCMatrix_to_scipy(a.sparse(_n));
            } else {
...
void add_majorana_sparse(py::class_<MajoranaOpT>& pyMajoranaOpClass) {
    pyMajoranaOpClass.def(
        "sparse",
        [](const MajoranaOpT& a, const std::optional<int>& n) {
            if (n.has_value()) {
...
void add_transforms(py::module_& m) {
    ...
    m.def("jw", py::overload_cast<const MajoranaString&>(&jordan_wigner),
          "Convert a Majorana polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("jw", py::overload_cast<const MajoranaPolynomial&>(&jordan_wigner),
          "Convert a Majorana polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("jw", py::overload_cast<const FermiString&>(&jordan_wigner),
          "Convert a Fermi polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("jw", py::overload_cast<const FermiPolynomial&>(&jordan_wigner),
          "Convert a Fermi polynomial to a Pauli polynomial via the Jordan-Wigner mapping");

    // Pauli to Fermi
    m.def("rjw", py::overload_cast<const PauliString&>(&reverse_jordan_wigner),
          "Convert a Pauli polynomial to a Fermi polynomial via the reverse Jordan-Wigner mapping");
    m.def("rjw", py::overload_cast<const PauliPolynomial&>(&reverse_jordan_wigner),
          "Convert a Pauli polynomial to a Fermi polynomial via the reverse Jordan-Wigner mapping");

    // To Pauli
    m.def("topauli", py::overload_cast<const MajoranaString&>(&jordan_wigner),
          "Convert a Majorana polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("topauli", py::overload_cast<const MajoranaPolynomial&>(&jordan_wigner),
          "Convert a Majorana polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("topauli", py::overload_cast<const FermiString&>(&jordan_wigner),
          "Convert a Fermi polynomial to a Pauli polynomial via the Jordan-Wigner mapping");
    m.def("topauli", py::overload_cast<const FermiPolynomial&>(&jordan_wigner),
          "Convert a Fermi polynomial to a Pauli polynomial via the Jordan-Wigner mapping");

    // To Fermi
    m.def("tofermi", py::overload_cast<const PauliString&>(&reverse_jordan_wigner),
          "Convert a Pauli polynomial to a Fermi polynomial via the reverse Jordan-Wigner mapping");
    m.def("tofermi", py::overload_cast<const PauliPolynomial&>(&reverse_jordan_wigner),
          "Convert a Pauli polynomial to a Fermi polynomial via the reverse Jordan-Wigner mapping");
    m.def("tofermi", py::overload_cast<const MajoranaString&>(&majorana_to_fermi),
          "Convert a Majorana polynomial a Fermi polynomial");
    m.def("tofermi", py::overload_cast<const MajoranaPolynomial&>(&majorana_to_fermi),
          "Convert a Majorana polynomial a Fermi polynomial");

    // To Majorana
    m.def("tomajorana", py::overload_cast<const FermiString&>(&fermi_to_majorana),
          "Convert a Fermi polynomial to Majorana polynomial");
    m.def("tomajorana", py::overload_cast<const FermiPolynomial&>(&fermi_to_majorana),
          "Convert a Fermi polynomial to Majorana polynomial");
    m.def(
        "tomajorana", [](const PauliString& a) { return MajoranaPolynomial(pauli_to_majorana(a)); },
        "Convert a Pauli polynomial to a Majorana polynomial");
    m.def("tomajorana", py::overload_cast<const PauliPolynomial&>(&pauli_to_majorana),
          "Convert a Pauli polynomial to a Majorana polynomial");
}
...
void add_states(py::module_& m) {
    py::class_<FockState>(m, "FockState")
        ...
        .def("__call__",
             py::overload_cast<const FermiPolynomial&>(&FockState::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const MajoranaString&>(&FockState::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const MajoranaPolynomial&>(&FockState::operator(), py::const_))
        .def("occ", &FockState::occ)
        .def(
            "vec",
            [](const FockState& state, const int& n, const std::optional<int>& nocc) {
                if (nocc.has_value()) {
    ...
    py::class_<QubitProductState>(m, "QubitProductState")
        ...
        .def("__call__",
             py::overload_cast<const PauliString&>(&QubitProductState::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&QubitProductState::operator(), py::const_))
        .def("up", &QubitProductState::up)
        .def(
            "vec",
            [](const QubitProductState& state, const int& n, const std::optional<int>& nup) {
                if (nup.has_value()) {
...
void add_pauli_gates(py::module_& m) {
    py::class_<pauli_gates::H>(m, "H")
        ...
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::H::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::H::operator(), py::const_))
        .def("__repr__", &pauli_gates::H::to_string)
        .def("__str__", &pauli_gates::H::to_string)
        .def_property_readonly("qubits",
                               [](const pauli_gates::H& gate) { return std::vector<int>{gate.i}; })
    ...
    py::class_<pauli_gates::S>(m, "S")
        .def(py::init<const int&>())
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::S::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::S::operator(), py::const_))
        .def("__repr__", &pauli_gates::S::to_string)
        .def("__str__", &pauli_gates::S::to_string)
        .def_property_readonly("qubits",
                               [](const pauli_gates::S& gate) { return std::vector<int>{gate.i}; })
    ...
    py::class_<pauli_gates::CNOT>(m, "CNOT")
        ...
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::CNOT::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::CNOT::operator(), py::const_))
        .def("__repr__", &pauli_gates::CNOT::to_string)
        .def("__str__", &pauli_gates::CNOT::to_string)
        .def_property_readonly(
            "qubits",
            [](const pauli_gates::CNOT& gate) { return std::vector<int>{gate.i, gate.j}; })
    ...
    py::class_<pauli_gates::SWAP>(m, "SWAP")
        .def(py::init<const int&, const int&>())
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::SWAP::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::SWAP::operator(), py::const_))
        .def("__repr__", &pauli_gates::SWAP::to_string)
        .def("__str__", &pauli_gates::SWAP::to_string)
        .def_property_readonly(
            "qubits",
            [](const pauli_gates::SWAP& gate) { return std::vector<int>{gate.i, gate.j}; })
    ...
    py::class_<pauli_gates::CZ>(m, "CZ")
        .def(py::init<const int&, const int&>())
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::CZ::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::CZ::operator(), py::const_))
        .def("__repr__", &pauli_gates::CZ::to_string)
        .def("__str__", &pauli_gates::CZ::to_string)
        .def_property_readonly(
            "qubits", [](const pauli_gates::CZ& gate) { return std::vector<int>{gate.i, gate.j}; })
    ...
    py::class_<pauli_gates::ROT>(m, "ROT")
        ...
        .def("__call__",
             py::overload_cast<const PauliString&>(&pauli_gates::ROT::operator(), py::const_))
        .def("__call__",
             py::overload_cast<const PauliPolynomial&>(&pauli_gates::ROT::operator(), py::const_))
        .def_property_readonly("qubits",
                               [](const pauli_gates::ROT& gate) { return gate.ps.support_set(); })
...
void add_majorana_propagation(py::module_& m) {
    py::class_<majorana_gates::MROT>(m, "MROT")
        ...
        .def("__call__", py::overload_cast<const MajoranaString&>(&majorana_gates::MROT::operator(),
                                                                  py::const_))
        .def("__call__", py::overload_cast<const MajoranaPolynomial&>(
                             &majorana_gates::MROT::operator(), py::const_))
        .def_property_readonly(
            "qubits", [](const majorana_gates::MROT& gate) { return gate.ms.support_set(); })
...
void add_gen(py::module_& m) {
    ...
    m.def("paulistrings",
          py::overload_cast<int, int,
                            std::function<bool(const std::vector<int>&, const std::vector<char>&)>>(
              &paulistrings));
    ...
    m.def("majoranastrings",
          py::overload_cast<int, int, std::function<bool(const std::vector<int>&)>>(
              &majoranastrings));
}
...
PYBIND11_MODULE(ffcore, m, py::mod_gil_not_used()) {
    ...
    py::class_<PauliString> pyPauliString =
        py::class_<PauliString>(m, "PauliString")
            ...
            .def(
                "degree",
                [](const PauliString& a, const std::optional<char>& v) {
                    return v.has_value() ? a.degree(v.value()) : a.degree();
                },
                py::arg("v") = py::none())
            .def("extent", &PauliString::extent)
            .def("commutes",
                 py::overload_cast<const PauliString&>(&PauliString::commutes, py::const_))
            .def("commutes",
                 py::overload_cast<const PauliPolynomial&>(&PauliString::commutes, py::const_))
            .def("commutator",
                 [](const PauliString& a, const PauliString& b) {
                     return PauliPolynomial(a.commutator(b));
                 })
            .def("commutator",
                 py::overload_cast<const PauliPolynomial&>(&PauliString::commutator, py::const_))
            ...
            // .def("sparse", [](const PauliString& a, int n) { return
            // CSCMatrix_to_scipy(a.sparse(n)); })

            .def("tofermi", [](const PauliString& p) { return reverse_jordan_wigner(p); })
            .def("tomajorana",
                 [](const PauliString& p) { return MajoranaPolynomial(pauli_to_majorana(p)); })
            ...
            .def("__add__",
                 [](const PauliString& a, ff_complex b) {
                     return b == ff_complex(0, 0) ? a : (a + PauliPolynomial(b));
                 })
            ...
            .def("__mul__",
                 [](const PauliString& a, ff_complex b) { return PauliPolynomial(a * b); })
            .def("__mul__",
                 [](const PauliString& a, const PauliString& b) { return PauliPolynomial(a * b); })
            .def("__mul__", [](const PauliString& a,
                               const PauliPolynomial& b) { return PauliPolynomial(a * b); })

            .def("__rsub__",
                 [](const PauliString& a, ff_complex b) {
                     return PauliPolynomial(b == ff_complex(0, 0) ? -a : (PauliPolynomial(b) - a));
                 })
            .def("__radd__",
                 [](const PauliString& a, ff_complex b) {
                     return PauliPolynomial(b == ff_complex(0, 0) ? a : (PauliPolynomial(b) + a));
            ...
            .def("__rmul__",
                 [](const PauliString& a, ff_complex b) { return PauliPolynomial(b * a); });

    py::class_<PauliPolynomial> pyPauliPolynomial =
        py::class_<PauliPolynomial>(m, "PauliPolynomial")
            .def(py::init<>())
            ...
            .def(
                "degree",
                [](const PauliPolynomial& a, const std::optional<char>& v) {
                    return v.has_value() ? a.degree(v.value()) : a.degree();
                },
                py::arg("v") = py::none())
            .def("dagger", &PauliPolynomial::dagger)
            .def("coefficient",
                 py::overload_cast<const PauliString&>(&PauliPolynomial::coefficient, py::const_))
            .def("coefficient",
                 [](const PauliPolynomial& a, const std::vector<std::pair<int, char>>& indices) {
                     return a.coefficient(PauliString(indices));
                 })
            .def("coefficient",
                 [](const PauliPolynomial& a, const std::string& ps) {
                     return a.coefficient(PauliString(ps));
                 })
            .def("support", &PauliPolynomial::support_set)
            /*.def("sparse", [](const PauliPolynomial& a) { return CSCMatrix_to_scipy(a.sparse());
            }) .def("sparse", [](const PauliPolynomial& a, int n) { return
            CSCMatrix_to_scipy(a.sparse(n)); }) .def("sparse", [](const PauliPolynomial& a, int n,
            int nup) { return CSCMatrix_to_scipy(a.sparse(n, nup)); })*/
    ...
    py::class_<FermiString> pyFermiString =
        py::class_<FermiString>(m, "FermiString")
            .def(py::init<>())
            ...
            .def(
                "degree",
                [](const FermiString& a, const std::optional<int>& v) {
                    if (v.has_value()) {
                ...
                py::arg("v") = py::none())
            .def("permute",
                 [](const FermiString& a, const std::vector<int>& perm) {
                     return FermiPolynomial(a.permute(perm));
                 })
            .def("commutes",
                 py::overload_cast<const FermiString&>(&FermiString::commutes, py::const_))
            .def("commutes",
                 py::overload_cast<const FermiPolynomial&>(&FermiString::commutes, py::const_))
            .def("commutator",
                 py::overload_cast<const FermiString&>(&FermiString::commutator, py::const_))
            .def("commutator",
                 py::overload_cast<const FermiPolynomial&>(&FermiString::commutator, py::const_))
            ...
            // .def("sparse", [](const FermiString& a, int n) { return
            // CSCMatrix_to_scipy(a.sparse(n)); })
            ...
            .def("__div__", [](const FermiString& a,
                               const ff_complex& b) { return FermiPolynomial(a * (1.0 / b)); })
            ...
            .def("__mul__",
                 [](const FermiString& a, const ff_complex& b) { return FermiPolynomial(a * b); })
            .def("__mul__",
                 [](const FermiString& a, const FermiString& b) { return FermiPolynomial(a * b); })
            .def("__mul__", [](const FermiString& a,
                               const FermiPolynomial& b) { return FermiPolynomial(a * b); })
            ...
            .def("__rmul__",
                 [](const FermiString& a, const ff_complex& b) { return FermiPolynomial(b * a); })

            .def("__str__", &FermiString::to_compact_string)
            .def("__repr__", &FermiString::to_compact_string);

    py::class_<FermiPolynomial> pyFermiPolynomial =
        py::class_<FermiPolynomial>(m, "FermiPolynomial")
            .def(py::init<>())
            .def(py::init<const ff_complex&>())
            ...
            .def(
                "degree",
                [](const FermiPolynomial& a, const std::optional<int>& v) {
                    return v.has_value() ? a.degree(v.value()) : a.degree();
                },
                py::arg("v") = py::none())
            .def("dagger", &FermiPolynomial::dagger)
            .def("coefficient",
                 py::overload_cast<const FermiString&>(&FermiPolynomial::coefficient, py::const_))
            .def("coefficient",
                 [](const FermiPolynomial& a, const std::string& fs) {
                     return a.coefficient(FermiString(fs));
                 })
            .def("coefficient",
                 [](const FermiPolynomial& a, const std::vector<std::pair<int, bool>>& fs) {
                     return a.coefficient(FermiString(fs));
                 })
    ...
    py::class_<MajoranaString> pyMajoranaString =
        py::class_<MajoranaString>(m, "MajoranaString")
            .def(py::init<>())
            ...
            .def("commutes",
                 py::overload_cast<const MajoranaString&>(&MajoranaString::commutes, py::const_))
            .def("commutes", py::overload_cast<const MajoranaPolynomial&>(&MajoranaString::commutes,
                                                                          py::const_))
            .def("commutator",
                 [](const MajoranaString& a, const MajoranaString& b) {
                     return MajoranaPolynomial(a.commutator(b));
                 })
            .def("commutator", py::overload_cast<const MajoranaPolynomial&>(
                                   &MajoranaString::commutator, py::const_))
            .def("permute",
                 [](const MajoranaString& a, const std::vector<int>& perm) {
                     return MajoranaPolynomial(a.permute(perm));
                 })
            ...
            .def("__div__", [](const MajoranaString& a,
                               const ff_complex& b) { return MajoranaPolynomial(a * (1.0 / b)); })
            ...
            .def("__add__",
                 [](const MajoranaString& a, const MajoranaPolynomial& b) { return a + b; })
            ...
            .def("__mul__", [](const MajoranaString& a,
                               const ff_complex& b) { return MajoranaPolynomial(a * b); })
            .def("__mul__", [](const MajoranaString& a,
                               const MajoranaString& b) { return MajoranaPolynomial(a * b); })
            .def("__mul__", [](const MajoranaString& a,
                               const MajoranaPolynomial& b) { return MajoranaPolynomial(a * b); })
            ...
            .def("__rmul__", [](const MajoranaString& a,
                                const ff_complex& b) { return MajoranaPolynomial(b * a); })

            .def("__str__", &MajoranaString::to_compact_string)
            .def("__repr__", &MajoranaString::to_compact_string);

    py::class_<MajoranaPolynomial> pyMajoranaPolynomial =
        py::class_<MajoranaPolynomial>(m, "MajoranaPolynomial")
            .def(py::init<>())
            .def(py::init<const ff_complex&>())
            ...
            .def("coefficient", py::overload_cast<const MajoranaString&>(
                                    &MajoranaPolynomial::coefficient, py::const_))
            .def("coefficient",
                 [](const MajoranaPolynomial& a, const std::string& ms) {
                     return a.coefficient(MajoranaString(ms));
                 })
            .def("coefficient",
                 [](const MajoranaPolynomial& a, const std::vector<int>& ms) {
                     return a.coefficient(MajoranaString(ms));
                 })
            .def("support", &MajoranaPolynomial::support_set)
            // .def("sparse", [](const MajoranaPolynomial& a) { return
            // CSCMatrix_to_scipy(a.sparse()); })
