...
MajoranaPolynomial _propagate(const MajoranaCircuit& circuit, const MajoranaPolynomial& obs,
                              FilterPred pred, const ff_float& mincoeff = 0) {
    MajoranaPolynomial ret(obs);
    int circuit_size = circuit.size();
    for (int i = circuit_size - 1; i >= 0; i--) {
        circuit[i].apply_inplace(ret, pred);
        if (mincoeff > 0) {
            std::erase_if(ret.terms, [&mincoeff](const auto& term) {
...
MajoranaPolynomial propagate(const MajoranaCircuit& circuit, const MajoranaPolynomial& obs,
...
MajoranaPolynomial propagate(const MajoranaCircuit& circuit, const MajoranaPolynomial& obs,
    ...
    return _propagate(
        circuit, obs, [&maxdegree](const MajoranaString& a) { return a.degree() <= maxdegree; },
        mincoeff);
}

MajoranaPolynomial propagate(const MajoranaCircuit& circuit, const MajoranaString& obs,
                             const ff_float& mincoeff = 0) {
