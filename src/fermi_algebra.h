...
struct FermiString {
    ...
    FermiString(const std::vector<std::pair<int, bool>>& str) {
        ...
        for (int i = 0; i < str_size; ++i) {
            ...
            if (str[i].first < 0 || str[i].first >= ff_ulong::DIGITS) {
                throw_error("Invalid FermiString (mode index "
                            << str[i].first << " invalid, should be between 0 and "
                            << ff_ulong::DIGITS - 1 << ")");
            }
            if (str[i].second == 1) {
                cre.set(str[i].first);
    ...
    FermiString(const std::vector<int>& cre_seq, const std::vector<int>& ann_seq) : cre(0), ann(0) {
        ...
        for (i = 0; i < cre_seq.size(); i++) {
            ...
            if (cre_seq[i] >= ff_ulong::DIGITS) {
                throw_error("Invalid FermiString (mode index "
                            << cre_seq[i] << " invalid, should be between 0 and "
                            << ff_ulong::DIGITS - 1 << ")");
            }
            cre.set(cre_seq[i]);
        }
        for (i = 0; i < ann_seq.size(); i++) {
            ...
            if (ann_seq[i] >= ff_ulong::DIGITS) {
                throw_error("Invalid FermiString (mode index "
                            << ann_seq[i] << " invalid, should be between 0 and "
                            << ff_ulong::DIGITS - 1 << ")");
            }
            ann.set(ann_seq[i]);
        }
    ...
    int extent() const { return ff_ulong::DIGITS - (cre | ann).countl_zero(); };
...
struct FermiStringHash {
    ...
    std::uint64_t operator()(const FermiString& a) const { return a.hash(); }
};

// Define a total order on FermiString
bool operator<(const FermiString& a, const FermiString& b) {
    return std::forward_as_tuple(a.ann | a.cre, a.cre, a.ann) <
           std::forward_as_tuple(b.ann | b.cre, b.cre, b.ann);
}
...
struct FermiMonomial {
    ...
    FermiMonomial(const std::vector<int>& cre_seq, const std::vector<int>& ann_seq,
                  ff_complex coeff)
        : s(cre_seq, ann_seq), coeff(coeff) {};
    FermiMonomial(const ff_ulong& cre, const ff_ulong& ann, ff_complex coeff)
        : s(cre, ann), coeff(coeff) {};
    ...
    FermiMonomial& operator*=(const ff_complex& b) {
        coeff *= b;
        return *this;
    }
    FermiMonomial permute(const std::vector<int>& map) const;
...
struct FermiPolynomial {
    ...
    FermiPolynomial() {};
    explicit FermiPolynomial(const ff_complex& v) {
        terms[FermiString()] = v;
    ...
    FermiPolynomial(const std::vector<int>& cre_seq, const std::vector<int>& ann_seq,
                    ff_complex coeff) {
            ...
            for (int action = 1; action >= 0; action--) {
                while (j < str_size && str[j].second == action) {
                    ...
                    if (loc < 0 || loc >= ff_ulong::DIGITS) {
                        throw_error("Invalid FermiString (mode index "
                                    << loc << " invalid, should be between 0 and "
                                    << ff_ulong::DIGITS - 1 << ")");
                    }
    ...
    FermiPolynomial(const std::vector<std::pair<int, bool>>& str) : FermiPolynomial(str, 1.0) {}
    ...
    bool is_hermitian() const {
        ...
        for (const auto& [x, v] : terms) {
            if (v != ff_complex(0, 0)) {
                xd = x.dagger();
                try {
                    if (terms.at(xd.fermi_string()) != xd.coefficient() * std::conj(v))
                        return false;
    ...
    // In-place addition
    FermiPolynomial& operator+=(const ff_complex& b) {
        if (b != ff_complex(0, 0)) {
            terms[FermiString()] += b;
            inplace_compress(FermiString());
        }
        return *this;
    };
    FermiPolynomial& operator+=(const FermiString& b) {
        terms[b] += 1;
        inplace_compress(b);
        return *this;
    };
    FermiPolynomial& operator+=(const FermiMonomial& b) {
        terms[b.s] += b.coeff;
...
FermiPolynomial operator*(const FermiString& x, const FermiString& y) {
    ...
    FermiMonomial default_monomial = FermiMonomial(
        x.cre | (y.cre & ~W0), (x.ann & ~W0) | y.ann,
        epsW0 *
            scalar_utils::m1pow(merge_parity(x.cre, y.cre & ~W0) + merge_parity(x.ann & ~W0, y.ann))
    ...
    for (int wbar = ScapTminusW0.begin(); wbar != ScapTminusW0.end();
         wbar = ScapTminusW0.next(wbar)) {
        ...
        for (i = count; i < 2 * count; i++) {
            ...
            terms[i] = FermiMonomial(x.cre | TminusW, SminusW | y.ann, cW);
...
FermiMonomial operator*(const FermiMonomial& a, const ff_complex& b) {
    return FermiMonomial(a.s, a.coeff * b);
}
FermiMonomial operator*(const ff_complex& a, const FermiMonomial& b) {
    return FermiMonomial(b.s, b.coeff * a);
}

FermiPolynomial operator+(const ff_complex& a, const FermiString& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const ff_complex& a, const FermiMonomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const ff_complex& a, const FermiPolynomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}

FermiPolynomial operator+(const FermiString& a, const ff_complex& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiMonomial& a, const ff_complex& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiPolynomial& a, const ff_complex& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}

FermiPolynomial operator+(const FermiString& a, const FermiString& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiString& a, const FermiMonomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiString& a, const FermiPolynomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiMonomial& a, const FermiString& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiMonomial& a, const FermiMonomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiMonomial& a, const FermiPolynomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}

FermiPolynomial operator+(const FermiPolynomial& a, const FermiString& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiPolynomial& a, const FermiMonomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
FermiPolynomial operator+(const FermiPolynomial& a, const FermiPolynomial& b) {
    FermiPolynomial c(a);
    c += b;
    return c;
}
...
FermiPolynomial operator*(const ff_complex& b, const FermiPolynomial& a) {
    FermiPolynomial c(a);
    c *= b;
    return c;
}
...
FermiPolynomial operator*(const FermiString& a, const FermiMonomial& b) {
    return b.coefficient() * (a * b.fermi_string());
}
FermiPolynomial operator*(const FermiMonomial& a, const FermiString& b) {
    return a.coefficient() * (a.fermi_string() * b);
}
FermiPolynomial operator*(const FermiMonomial& a, const FermiMonomial& b) {
    return a.coefficient() * b.coefficient() * (a.fermi_string() * b.fermi_string());
}
FermiPolynomial operator*(const FermiPolynomial& a, const FermiPolynomial& b) {
    FermiPolynomial c(a);
    c *= b;
...
FermiPolynomial FermiPolynomial::commutator(const FermiPolynomial& b) const {
    return ((*this) * b - b * (*this));
}
FermiPolynomial FermiPolynomial::commutator(const FermiString& b) const {
    return commutator(FermiPolynomial(b));
}
FermiPolynomial FermiPolynomial::commutator(const FermiMonomial& b) const {
    return commutator(FermiPolynomial(b));
}

FermiPolynomial FermiString::commutator(const FermiString& b) const {
    return ((*this) * b - b * (*this));
}
FermiPolynomial FermiString::commutator(const FermiMonomial& b) const {
    return b.coeff * commutator(b.s);
}
FermiPolynomial FermiString::commutator(const FermiPolynomial& b) const {
    return FermiPolynomial(*this).commutator(b);
}

FermiPolynomial FermiMonomial::commutator(const FermiString& b) const {
    return coeff * s.commutator(b);
}
FermiPolynomial FermiMonomial::commutator(const FermiMonomial& b) const {
    return (coeff * b.coeff) * s.commutator(b.s);
}
FermiPolynomial FermiMonomial::commutator(const FermiPolynomial& b) const {
    return FermiPolynomial(*this).commutator(b);
}
...
FermiMonomial FermiMonomial::permute(const std::vector<int>& map) const {
...
bool operator==(const FermiString& a, const ff_complex& b) {
    return a.cre == 0 && a.ann == 0 && b == ff_complex(1, 0);
}
bool operator==(const FermiString& a, const FermiString& b) {
    return a.cre == b.cre && a.ann == b.ann;
}
bool operator==(const FermiString& a, const FermiMonomial& b) {
    return a == b.s && b.coeff == ff_complex(1, 0);
}
bool operator==(const FermiString& a, const FermiPolynomial& b) { return FermiPolynomial(a) == b; }

bool operator==(const FermiMonomial& a, const ff_complex& b) {
    return a.s.cre == 0 && a.s.ann == 0 && a.coeff == b;
}
bool operator==(const FermiMonomial& a, const FermiString& b) {
    return a.s == b && a.coeff == ff_complex(1, 0);
}
bool operator==(const FermiMonomial& a, const FermiMonomial& b) {
    return a.s == b.s && a.coeff == b.coeff;
}
bool operator==(const FermiMonomial& a, const FermiPolynomial& b) {
    return FermiPolynomial(a) == b;
}
...
bool operator==(const FermiPolynomial& a, const FermiMonomial& b) {
    return a == FermiPolynomial(b);
}
...
std::ostream& operator<<(std::ostream& os, const FermiString& a) {
    return os << a.to_compact_string();
}
std::ostream& operator<<(std::ostream& os, const FermiMonomial& a) {
    return os << a.to_compact_string();
}
std::ostream& operator<<(std::ostream& os, const FermiPolynomial& p) {
