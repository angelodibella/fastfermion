...
std::string format_complex(const ff_complex& z) {
    if (z.imag() == 0) {
        ...
        if (z.imag() < 0) {
            return "(" + format_float(z.real()) + "-" + format_float(-z.imag()) +
                   ff_config.imaginary_symbol + ")";
        } else {
            return "(" + format_float(z.real()) + "+" + format_float(z.imag()) +
                   ff_config.imaginary_symbol + ")";
        }
...
std::string format_poly(const Poly& poly, int max_terms_to_show = 50, int max_line_length = 120) {
    ...
    for (auto& [x, v] : poly.terms) {
        if (i >= max_terms_to_show) {
            ...
            ret += (std::stringstream() << " + ... (" << poly.terms.size() - i << " more terms)")
                       .str();
            break;
        }
        ...
        if (i > 0) {
            ...
            std::string sep_op =
                (v.imag() == 0 && v.real() < 0) || (v.real() == 0 && v.imag() < 0) ? "-" : "+";
        ...
        if (v == ff_complex(1, 0)) {
            ...
            // the negative sign unless we are in the first term
            coeffstr =
                format_float(i == 0 ? v.imag() : std::abs(v.imag())) + ff_config.imaginary_symbol;
        } else {
            // Coefficient has real and imaginary components
            if (v.imag() < 0) {
                coeffstr = "(" + format_float(v.real()) + "-" + format_float(-v.imag()) +
                           ff_config.imaginary_symbol + ")";
            } else {
                coeffstr = "(" + format_float(v.real()) + "+" + format_float(v.imag()) +
                           ff_config.imaginary_symbol + ")";
