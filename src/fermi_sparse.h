...
CSCMatrix<ff_complex> _fermipoly_sparse(const FermiPolynomial& poly, int n,
                                        const Subspace& subspace) {
    // Returns a matrix of size dim x dim where dim = subspace.dim
    // If subspace = FullHilbertSpace, then dim = 2^n
    // Each row/column is indexed by a bitstring of length n. This bit string is read from *left to
    // right* i.e., the leftmost bit corresponds to whether mode *0* occupied, etc.
    //
    ...
    std::sort(ops_info.begin(), ops_info.end(),
              [](const FermiStringExtra& a, const FermiStringExtra& b) { return a.us < b.us; });
    ...
    for (j = 0, b = subspace.start; j < subspace_dim; j++, b = subspace.next(b)) {
        ...
        while (i < nops) {
            ...
            do {
                ...
                s = ops_info[i].s;
                if ((((~b) & s) == 0) && (b & u & (~s)) == 0) {
                    sgn_coeff = scalar_utils::m1pow(merge_parity(ops_info[i].s, b ^ ops_info[i].s) +
                                                    merge_parity(ops_info[i].u, b ^ us));
                    // sgn_coeff =
                    // merge_parity(b^ops_info[i].s,ops_info[i].s,true)*merge_parity(b^us,ops_info[i].u,true);
...
CSCMatrix<ff_complex> FermiPolynomial::sparse(int n) const {
    return _fermipoly_sparse(*this, n, FullHilbertSpace(n));
}
CSCMatrix<ff_complex> FermiPolynomial::sparse(int n, int nocc) const {
    return _fermipoly_sparse(*this, n, FixedCountSubspace(n, nocc));
}

CSCMatrix<ff_complex> FermiString::sparse(int n) const { return FermiPolynomial(*this).sparse(n); }
CSCMatrix<ff_complex> FermiString::sparse(int n, int nocc) const {
    return FermiPolynomial(*this).sparse(n, nocc);
}
