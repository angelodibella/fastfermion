...
void _fermi_to_majorana(const FermiString& a, MajoranaPolynomial& res,
                        const ff_complex& multiplier = 1,
                        const bool& only_keep_hermitian_terms = false) {
    ...
    ff_float global_coeff =
        ...
        scalar_utils::m1pow(deg_cre * (deg_cre - 1) / 2 + deg_ann * (deg_ann - 1) / 2 +
                            merge_parity(a.ann, a.cre));
    ...
    for (int j = supp.begin(); j != supp.end(); j = supp.next(j)) {
        ...
        jInS =
            !jInU ||
            a.ann.at(j);  // if j is not in U, it must be in S (save checking bit position at a.ann)
    ...
    for (z = 0; z < num_terms; z++) {
        bool hermitian_term =
            MajoranaMonomial(terms[z], coeffs[z]).dagger() == MajoranaMonomial(terms[z], coeffs[z]);
...
PauliMonomial majorana_to_pauli(const MajoranaString& a) {
    ...
    for (int i = a.alpha.begin(); i != a.alpha.end(); i = a.alpha.next(i)) {
        int j = i / 2;
        int r = i % 2;
        pauli_string_multiply(res, jpow,
                              // Z_0 ... Z_{j-1} X_j if r = 0
                              // Z_0 ... Z_{j-1} Y_j if r = 1
                              PauliString(ff_ulong::singleton(j), ff_ulong::range(j, r == 1)));
    }
...
PauliPolynomial majorana_to_pauli(const MajoranaPolynomial& p) { return jordan_wigner(p); }

PauliPolynomial jordan_wigner(const FermiString& a) { return jordan_wigner(fermi_to_majorana(a)); }
...
void _majorana_to_fermi(const MajoranaString& a, FermiPolynomial& res,
                        const ff_complex& multiplier = 1) {
    ...
    ff_complex global_coeff =
        multiplier * scalar_utils::m1pow(deg * (deg - 1) / 2) * scalar_utils::Ipow(_odd.popcount());
    ...
    for (int j = evenorodd.begin(); j != evenorodd.end(); j = evenorodd.next(j)) {
        ...
        for (z = count; z != 2 * count; z++) {
            ...
            cre = (inEven && inOdd) ? (terms[z - count].cre | singj)
...
MajoranaMonomial pauli_to_majorana(const PauliString& a) {
    ...
    for (int j = supp.begin(); j != supp.end(); j = supp.next(j)) {
        ...
        if (!xory && !yorz) {
            ...
            // Y
            sqm = MajoranaString(ff_dbl_ulong::range(2 * j, false) |
                                 ff_dbl_ulong::singleton(2 * j + 1));
            jpow -= j;
        } else if (!xory && yorz) {
            // Z
            // 3 is '11' in binary
