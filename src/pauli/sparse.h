...
CSCMatrix<ff_complex> _paulipoly_sparse(const PauliPolynomial& poly, int n,
    ...
    std::sort(ops_info.begin(), ops_info.end(),
              [](const PauliStringExtra& a, const PauliStringExtra& b) {
                  return a.xory_flipped < b.xory_flipped;
              });
...
CSCMatrix<ff_complex> PauliString::sparse(int n, int nup) const {
    return PauliPolynomial(*this).sparse(n, nup);
}

CSCMatrix<ff_complex> PauliMonomial::sparse(int n) const {
    return PauliPolynomial(*this).sparse(n);
}
