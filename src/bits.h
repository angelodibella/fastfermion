...
struct BitSet {
    ...
    int popcount() const {
        int ret = 0;
        for (std::size_t i = 0; i < N; ++i) ret += std::popcount(words[i]);
        return ret;
    }
    ...
    std::uint64_t to_ullong() const { return words[0]; }

    std::vector<int> support() const {
        // Returns positions of activated bits in increasing order (from least significant bit to
        // most significant one) The length of the returned vector is equal to popcount()
        std::vector<int> supp;
        int len = popcount();
        supp.reserve(len);
        int cur_bit;
    ...
    std::vector<int> rsupport() const {
        // Returns positions of activated bits in reverse order (from most significant bit to least
        // significant one) The length of the returned vector is equal to popcount()
        std::vector<int> supp;
        int len = popcount();
        supp.reserve(len);
        int cur_bit;
...
template <int N>
BitSet<N> operator<<(const BitSet<N>& a, int shift) {
    BitSet<N> c(a);
    c <<= shift;
    return c;
}
...
template <int N>
bool operator==(const BitSet<N>& a, std::uint64_t b) {
    return a.words == BitSet<N>(b).words;
}
template <int N>
bool operator==(std::uint64_t a, const BitSet<N>& b) {
    return BitSet<N>(a).words == b.words;
}
template <int N>
bool operator<(const BitSet<N>& a, const BitSet<N>& b) {
    int i = N - 1;
