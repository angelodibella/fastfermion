...
// Pauli string algebra -- PauliString, PauliMonomial, PauliPolynomial.
...
// Two-bit encoding per qubit: I=(0,0), X=(1,0), Y=(1,1), Z=(0,1)
struct PauliString {
    ...
    PauliString(const std::vector<int>& loc, const std::vector<char>& actions) : xory(0), yorz(0) {
        ...
        for (std::size_t i = 0; i < loc.size(); i++) {
            if (loc[i] >= 0 && loc[i] < ff_ulong::DIGITS) {
                if (actions[i] == 'I' || actions[i] == 'X' || actions[i] == 'Y' ||
                    actions[i] == 'Z') {
                    if (actions[i] == 'X' || actions[i] == 'Y') xory.set(loc[i]);
                    if (actions[i] == 'Y' || actions[i] == 'Z') yorz.set(loc[i]);
                } else {
                    throw_error("Invalid PauliString (symbol '" << actions[i]
                                                                << "' not recognized)");
                }
            } else {
                throw_error("Invalid PauliString (qubit index "
                            << loc[i] << " invalid, should be between 0 and "
                            << ff_ulong::DIGITS - 1 << ")");
            }
        }
    }
        ...
        for (int i = 0; i < str_size; ++i) {
            ...
            for (int j = 0; j < str_size; ++j) {
                if (j != i && str[j].first == loc) {
                    throw_error("Invalid PauliString (qubit index " << loc
            ...
            if (loc >= 0 && loc < ff_ulong::DIGITS) {
                ...
                throw_error("Invalid PauliString (qubit index "
                            << loc << " invalid, should be between 0 and " << ff_ulong::DIGITS - 1
                            << ")");
            }
        }
    }
    ...
    std::vector<int> support_set() const { return (xory | yorz).support(); }
    std::vector<std::pair<int, char>> indices() const {
        ...
        for (int pos = a1.begin(); pos != a1.end(); pos = a1.next(pos)) {
            ...
            bool q = yorz.at(pos);
            if (p && q)
                ret.push_back(std::make_pair(pos, 'Y'));
            else if (p && !q)
                ret.push_back(std::make_pair(pos, 'X'));
            else if (!p && q)
                ret.push_back(std::make_pair(pos, 'Z'));
        }
        return ret;
    }
    ...
    int degree(const char& v) const {
        if (v == 'X' || v == 'x')
            ...
            throw_error("Invalid argument: should be either 'X', 'Y', or 'Z' or empty char");
    }
    // Index of highest nontrivial qubit + 1
    int extent() const { return ff_ulong::DIGITS - (xory | yorz).countl_zero(); };
    // Odd number of Y factors
    bool is_skew_symmetric() const { return degree_y() % 2 == 1; }
    ...
    PauliString permute(const std::vector<int>& map) const {
        return PauliString(move_bits(xory, map), move_bits(yorz, map));
    }

    bool commutes(const ff_complex& b) const { return true; }
    // Symplectic inner product parity: counts mismatched X-Z overlaps mod 2
...
// In-place product a <- a*b, accumulating phase in jpow (coefficient is i^jpow).
// Converts to/from XZ representation via Y-count adjustment to apply the product rule.
void pauli_string_multiply(PauliString& a, int& jpow, const PauliString& b) {
    jpow += (a.xory & a.yorz).popcount() + (b.xory & b.yorz).popcount() +
            2 * (a.yorz & b.xory).popcount();
    a.xory ^= b.xory;
    a.yorz ^= b.yorz;
...
bool operator<(const PauliString& a, const PauliString& b) {
    return std::forward_as_tuple(a.xory | a.yorz, a.yorz, a.xory) <
           std::forward_as_tuple(b.xory | b.yorz, b.yorz, b.xory);
}

struct PauliStringHash {
    using is_avalanching = void;
    std::uint64_t operator()(const PauliString& a) const { return a.hash(); }
};

// A PauliMonomial is a PauliString together with a coefficient
struct PauliMonomial {
    PauliString s;  // Pauli string (the operator part)
    ff_complex coeff;

    PauliMonomial()
        : s(), coeff(0) {}  // NOT SURE THIS SHOULD BE THE RIGHT BEHAVIOR: Because then
                            // PauliString() == I and PauliMonomial() == 0
    ...
    PauliMonomial(const std::vector<int>& loc, const std::vector<char>& actions)
        : s(loc, actions), coeff(1) {}
    PauliMonomial(const std::vector<int>& loc, const std::vector<char>& actions, ff_complex coeff)
        : s(loc, actions), coeff(coeff) {}
    ...
    PauliMonomial permute(const std::vector<int>& map) const {
        return PauliMonomial(s.permute(map), coeff);
    }
    ...
    std::string to_string(int n = 0) const {
        return std::to_string(coeff.real()) + "+" + std::to_string(coeff.imag()) + "i " +
               s.to_string(n);
    }
    std::string to_compact_string() const {
        return format_complex(coeff) + " " + s.to_compact_string();
    }
    PauliMonomial& operator*=(const ff_float b) {
        coeff *= b;
        return *this;
    }
    bool commutes(const ff_complex& b) const { return true; }
...
struct PauliPolynomial {
    PauliPolynomialMap terms;  // maps PauliString -> coefficient
    PauliPolynomial() {}
    ...
    PauliPolynomial(const std::vector<std::pair<int, char>>& str, const ff_complex& coeff) {
        // Initialize using a sequence of location and actions (a la QubitOperator from OpenFermion)
        // e.g., str = ((0,'X'),(1,'Y'))
        // Note that str may contain the same location multiple times. In this case we have to keep
        // track of the sign.
        PauliString u;
        int jpow = 0;
        int str_size = str.size();
        for (int i = 0; i < str_size; ++i) {
            pauli_string_multiply(
                u, jpow,
                PauliString(std::vector<std::pair<int, char>>{{str[i].first, str[i].second}}));
        }
    ...
    // Remove exact-zero entry (used after in-place addition/subtraction)
    ...
    PauliPolynomial& operator+=(const PauliString& b) {
        terms[b] += 1;
        inplace_compress(b);
        return *this;
    };
    PauliPolynomial& operator+=(const PauliMonomial& b) {
        terms[b.s] += b.coeff;
        inplace_compress(b.s);
        return *this;
    };
    ...
    int extent() const {
        ...
        for (auto& [x, v] : terms) {
            int xli = x.extent();
            if (v != ff_complex(0, 0) && xli > l) l = xli;
        }
        return l;
    }
    ...
    // Remove terms with |coeff| <= threshold
    ...
    // Remove terms with degree > k (weight truncation)
    ...
    // <0^n|A|0^n>: sum coefficients of I/Z-only terms
...
PauliPolynomial operator+(const PauliString& a, const ff_complex& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliString& a, const PauliString& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliString& a, const PauliMonomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliString& a, const PauliPolynomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}

PauliPolynomial operator+(const PauliMonomial& a, const ff_complex& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliMonomial& a, const PauliString& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliMonomial& a, const PauliMonomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliMonomial& a, const PauliPolynomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}

PauliPolynomial operator+(const ff_complex& a, const PauliPolynomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliPolynomial& a, const ff_complex& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliPolynomial& a, const PauliString& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
PauliPolynomial operator+(const PauliPolynomial& a, const PauliMonomial& b) {
    PauliPolynomial c(a);
    c += b;
    return c;
}
...
bool operator==(const PauliPolynomial& a, const PauliMonomial& b) {
    return a == PauliPolynomial(b);
}

// Unary negation

PauliMonomial operator-(const PauliString& a) { return PauliMonomial(a, -1); };
PauliMonomial operator-(const PauliMonomial& a) {
    return PauliMonomial(a.pauli_string(), -a.coefficient());
};
PauliPolynomial operator-(const PauliPolynomial& a) {
    PauliPolynomial c(a);
    c *= -1;
    return c;
};
...
PauliMonomial operator*(const PauliMonomial& a, const ff_complex& b) {
    return PauliMonomial(a.s, a.coeff * b);
}
PauliMonomial operator*(const ff_complex& a, const PauliMonomial& b) {
    return PauliMonomial(b.s, b.coeff * a);
}
...
PauliPolynomial operator*(const ff_complex& b, const PauliPolynomial& a) {
    PauliPolynomial c(a);
    c *= b;
    return c;
}
...
PauliMonomial operator*(const PauliString& a, const PauliMonomial& b) {
    return PauliMonomial(a) * b;
};
PauliPolynomial operator*(const PauliString& a, const PauliPolynomial& b) {
    PauliPolynomial c(a);
    c *= b;
...
PauliMonomial operator*(const PauliMonomial& a, const PauliString& b) {
    return a * PauliMonomial(b);
};
PauliPolynomial operator*(const PauliMonomial& a, const PauliPolynomial& b) {
    PauliPolynomial c(a);
    c *= b;
...
bool operator==(const PauliString& a, const ff_complex& b) {
    return a.xory == 0 && a.yorz == 0 && b == ff_complex(1, 0);
}
bool operator==(const PauliString& a, const PauliString& b) {
    return a.xory == b.xory && a.yorz == b.yorz;
}
bool operator==(const PauliString& a, const PauliMonomial& b) {
    return a == b.s && b.coeff == ff_complex(1, 0);
}
bool operator==(const PauliString& a, const PauliPolynomial& b) { return PauliPolynomial(a) == b; }

bool operator==(const PauliMonomial& a, const ff_complex& b) {
    return a.s.xory == 0 && a.s.yorz == 0 && a.coeff == b;
}
bool operator==(const PauliMonomial& a, const PauliString& b) {
    return a.s == b && a.coeff == ff_complex(1, 0);
}
bool operator==(const PauliMonomial& a, const PauliMonomial& b) {
    return a.s == b.s && a.coeff == b.coeff;
}
bool operator==(const PauliMonomial& a, const PauliPolynomial& b) {
...
// For Pauli strings: [P,Q] = 0 if they commute, else [P,Q] = 2*P*Q.
...
PauliMonomial PauliString::commutator(const PauliMonomial& b) const {
    return b.coeff * commutator(b.s);
}
PauliMonomial PauliMonomial::commutator(const PauliString& b) const {
    return coeff * s.commutator(b);
}
PauliMonomial PauliMonomial::commutator(const PauliMonomial& b) const {
    return (coeff * b.coeff) * s.commutator(b.s);
}
...
PauliPolynomial PauliPolynomial::commutator(const PauliString& b) const {
    return commutator(PauliPolynomial(b));
}
PauliPolynomial PauliPolynomial::commutator(const PauliMonomial& b) const {
    return commutator(PauliPolynomial(b));
}

PauliPolynomial PauliString::commutator(const PauliPolynomial& b) const {
    return PauliPolynomial(*this).commutator(b);
}
PauliPolynomial PauliMonomial::commutator(const PauliPolynomial& b) const {
    return PauliPolynomial(*this).commutator(b);
}
...
std::ostream& operator<<(std::ostream& os, const PauliString& a) {
    return os << a.to_compact_string();
}
std::ostream& operator<<(std::ostream& os, const PauliMonomial& a) {
    return os << a.to_compact_string();
}
std::ostream& operator<<(std::ostream& os, const PauliPolynomial& p) {
    return os << p.to_compact_string();
}
